...
- Current active batch: Phase 7 Preview 2 release recovery — preserve the immutable hosted-tested tags, correct cross-platform archive determinism, and prepare the protected draft pre-release
...
**Protected archive failure and deterministic correction:** protected tag-push run [`33037140748`](https://github.com/Yurii-Tor/torproduction.addressables/actions/runs/33037140748) validated the tag, signature, exact candidate, package, and hosted matrix, then failed closed before release creation because Ubuntu rebuilt SHA-256 `192e6dc8e0b3dbef2f9e5a54fa341619eb7d0e7acf7f2fe5586100c6eb8f2f48` while the Windows-local evidence had recorded `9df4ceca1d9fd236fd6d262cfef1470a61fa6a536fa95a1e295ca4ad08093139`. GitHub still has zero releases, and neither Preview 2 tag changed. Reproduction proved the long-lived Windows worktree contained mixed CRLF/LF bytes even though `.gitattributes` and every committed Git blob use LF: packing a canonical `git archive` source on Windows produced Ubuntu's exact `192e6d...` archive. `New-PackageArchive.ps1` now copies source into a bounded temporary staging tree, normalizes only the repository-declared text extensions to LF at the byte level, packages that canonical tree, and verifies every extracted file byte against it. Two builds from the mixed Windows tree and one from canonical Git source all reproduce exact SHA-256 `192e6dc8e0b3dbef2f9e5a54fa341619eb7d0e7acf7f2fe5586100c6eb8f2f48`. That corrected exact archive independently passed install/EditMode `133/133`/warning/inertness/removal on Addressables `2.7.6` and `2.9.1`. The package source and signed-tag target are unchanged; this is release-infrastructure and checksum evidence only.

**Remaining gates:** commit the canonical archive builder/checksum correction, then pin it from the already protected manual recovery path without moving or recreating either tag. The recovery must continue to use all package/release-note inputs from the signed tag, use only the pinned canonical builder/checksum and hosted-validation helpers from the correction commit, reproduce `192e6d...`, and create only the draft. Then independently download and verify the exact draft assets, install the downloaded `.tgz` and signed Git URL, record final evidence, and stop. Public publication remains blocked until the owner manually tests that exact downloaded draft asset in a clean project and explicitly authorizes publication.
