...
**Status:** in progress at the protected GitHub pre-release recovery gate. The exact candidate and hosted Unity matrix passed, `phase-7-verified` and the verified signed `v0.1.0-preview.1` tag exist at that candidate, and no registry publication is authorized. The first protected release run failed before asset construction or release creation because of a cross-platform PowerShell overload-resolution defect; the narrowly scoped recovery described below is being applied without moving the signed tag or changing package content. OpenUPM and every other registry remain excluded.
