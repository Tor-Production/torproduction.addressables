...
- Planning: Complete
- Implementation: Phase 7 Preview 3 protected draft and exact consumer evidence verified; owner manual clean-project publication gate remains
...
- Current active batch: Phase 7 Preview 3 final manual gate — preserve Preview 2 and Preview 3 drafts/tags and wait for owner testing of the exact downloaded asset plus separate publication authorization
...
**Remaining Preview 3 manual publication gate:** in the owner's clean project at `C:\Users\morta\Documents\Projects\UnityProjects\EmptyProjectForTests`, first verify the exact Downloads archive SHA-256 above, then install it through Package Manager **Add package from tarball...** and confirm version `0.1.0-preview.3`. Confirm installation is inert and the Console has no hidden-sample metadata warning. Import **Basic Setup**, confirm the imported config shows **Category Label = basic-setup** and an empty **Required Labels** list, create default Addressables settings, explicitly activate the config, and verify validation has only expected pre-Apply missing-group/label warnings. Run Scene Analyze and confirm it proposes `Basic Setup Scenes`, bundled/content-update schemas, the `basic-setup` label, and the sample-scene entry; Apply, then confirm a second Analyze has no operations. Remove the imported sample and package and confirm unrelated project state is unchanged. Do not publish draft `v0.1.0-preview.3` until the owner reports this exact manual test passed and separately authorizes publication. Preview 2 also remains draft and must not be deleted or published. npm, OpenUPM, Unity Registry, Asset Store, and every other registry remain unauthorized.
