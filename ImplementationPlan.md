...
- Planning: Complete
- Implementation: Phase 7 Preview 2 corrective candidate, protected release automation, and draft pre-release preparation in progress
...
- Current active batch: Phase 7 Preview 2 correction — remove invalid hidden-sample root metadata, reject the Unity warning, revalidate the exact candidate, and prepare a protected draft pre-release
...
**Status:** Preview 1 failed the owner's manual clean-project release-quality gate and must not be published. Its draft release has been safely removed while its signed tag and the existing `phase-7-verified` evidence tag remain immutable. Preview 2 corrective implementation and complete local validation pass; candidate commit, the single paid hosted matrix, new verification tag, signed semantic tag, protected draft, exact draft-asset verification, and the owner's final manual gate remain. No registry publication is authorized. OpenUPM and every other registry remain excluded.
...
**Complete local Preview 2 evidence:** Unity `6000.0.78f1` development-host EditMode passed `133/133`. Disposable path lanes passed Addressables `2.7.6` EditMode `133/133` plus PlayMode `1/1` and Addressables `2.9.1` EditMode `133/133`; both verified package-import/removal inertness with `Samples~` physically excluded. Real UPM `Basic Setup` sample import/removal passed `133/133` independently on both `2.7.6` and `2.9.1`, preserved the unrelated sentinel and all sample GUIDs, and remained inert. Exact archive install/removal passed `133/133` and inertness independently on both versions. The deterministic archive and independently rebuilt repeat archive are byte-identical at SHA-256 `9df4ceca1d9fd236fd6d262cfef1470a61fa6a536fa95a1e295ca4ad08093139`; the archive contains no `package/Samples~.meta` and retains all five required inner metas. PVS again produced only the narrowly classified `PVP-20-1` Roslyn `System.TypeLoadException` in its unmodified XML-doc launcher; the same bundled checker exited `0` with empty stdout/stderr and every other applicable validation passed. Every Preview 2 log is free of the hidden-sample warning; all non-PVS logs are free of configured compiler/fatal patterns. Phase 0, package-manifest, release-readiness, PowerShell/JSON parsing, actionlint `1.7.12`, deterministic archive/content/checksum, and diff checks pass. Local evidence is under ignored `artifacts/phase7-preview2`. The candidate SHA is intentionally not recorded until this plan and the exact implementation are committed together.

**Remaining gates:** commit and push the exact candidate without including ignored evidence; confirm an ordinary push starts no workflow; dispatch exactly one paid hosted Unity matrix for that SHA; require both Addressables jobs and independently downloaded authoritative `133/133` XML reports to pass without the warning; create an additional clearly named Preview 2 verification tag without moving `phase-7-verified`; create and push a cryptographically signed `v0.1.0-preview.2` tag; approve the protected workflow so it creates only a draft pre-release; verify and install the exact downloaded draft `.tgz` plus signed Git URL; then stop. Public publication remains blocked until the owner manually tests that exact downloaded draft asset in a clean project and explicitly authorizes publication.
