...
**Status:** in progress at the final GitHub pre-release publication gate. The exact candidate and hosted Unity matrix passed; `phase-7-verified` and the verified signed `v0.1.0-preview.1` tag exist at that candidate; protected recovery created a verified draft pre-release; and both its downloaded `.tgz` and the signed Git-tag URL passed clean install/removal. No registry publication is authorized. OpenUPM and every other registry remain excluded.
