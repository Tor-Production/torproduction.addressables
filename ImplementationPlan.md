...
**Remaining gates:** push the protected recovery wiring that pins correction commit `4b19ae00ddb6b78ba6376a1f406ff7c4125ff0c8` without moving or recreating either tag. The recovery continues to use package and release-note inputs from the signed tag, and uses only the canonical builder/checksum, package validator, and hosted-validation helper from the pinned correction commit. It must reproduce `192e6d...` and create only the draft. Then independently download and verify the exact draft assets, install the downloaded `.tgz` and signed Git URL, record final evidence, and stop. Public publication remains blocked until the owner manually tests that exact downloaded draft asset in a clean project and explicitly authorizes publication.
