...
The current public preview candidate is `0.1.0-preview.3`. Its manifest version, changelog heading, archive filename, signed tag, and GitHub Release title must agree exactly.
...
3. Pass repository static checks, manifest/metadata checks, actionlint, API/assembly snapshots, EditMode tests, selected PlayMode validation, clean path install/removal for Addressables `2.7.6` and `2.9.1`, the real Basic Setup import/activation/Analyze/Apply/convergence/removal workflow on both lanes, Package Validation Suite with the recorded fail-closed XML-documentation fallback, direct bundled `FindMissingDocs.exe`, archive source/file-list validation, archive installation/removal, and inertness/clean-worktree checks.
...
6. Preserve all existing Phase 7 and Preview 2 tags, create a clearly named Preview 3 verification tag on the hosted-tested candidate, confirm it triggers no workflow, then create and locally verify the cryptographically signed `v0.1.0-preview.3` tag on the same commit.
...
10. Download the exact draft `.tgz`, independently verify its checksum and contents, and install both it and the signed Git tag in disposable Unity projects. Confirm compilation, all tests, the real Basic Setup activation/Analyze/Apply/convergence workflow, inert removal, and absence of the hidden `Samples~` metadata warning.
...
Ordinary pushes and pull requests never publish. Paid Unity validation remains `workflow_dispatch` only. Pushing the Preview 3 verification tag triggers nothing. The release workflow accepts only `v0.1.0-preview.3`, has top-level read-only permissions, and grants `contents: write` only to its protected release-creation job.
